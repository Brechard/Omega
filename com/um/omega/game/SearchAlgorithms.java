...
public class SearchAlgorithms {
	...
	public static String[] aspirationSearch(SimpleGame game, int delta, int maxDepth, SimpleGameController gameController) {
...
//		System.out.println("Is it possible to keep playing?" +(game.emptyCells.size() < 4)+ " and " +(game.playerToPlay == Main.gameController.getFirstPlayer()));
		...
		for(int depth = 1; depth <= maxDepth; depth++ ) {
			long alpha = - delta; 
			long beta = + delta;
			result = alphaBetaWithTT(game, depth, alpha, beta, gameController);
...
//			else hashes.add(h);
	...
	public static String[] alphaBetaWithTT(SimpleGame game, int depth, long alpha, long beta, SimpleGameController gameController) {

		long originalAlpha = alpha;
		long hash = gameController.getHash(game);
...
//			else hashes.add(h);
		...
		for(int child = 0; child < childGames.size(); child++) {
			valueHelper = alphaBetaWithTT(childGames.get(child), depth -1,(long) -beta,(long) -alpha, gameController);
